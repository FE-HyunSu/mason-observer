...
User Prompt
  ...
  → Stop Hook
  → Local JSONL (<project>/.mason-observer/events/*.jsonl)
  → inspect Command (/mason-observer:inspect-last, /mason-observer:inspect-session)
  → decision-analysis Skill (분석 절차 · 증거 등급 정의)
  → Mason Observer Report (observed / inferred / unknown 구분된 마크다운)
```
...
- **scripts/capture-event.js**: 각 Hook 이벤트를 stdin으로 받아, 이벤트별 allowlist로
  필드를 추출하고, `redact.js`로 마스킹한 뒤 `<project>/.mason-observer/events/<sessionId>.jsonl`
...
- **commands/*.md**: 사용자가 실행하는 `/mason-observer:inspect-last`,
  `/mason-observer:inspect-session`, `/mason-observer:status`. `read-events.js`를 호출해 원본
