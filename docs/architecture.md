...
User Prompt
  ...
  → inspect Command (/mason-recap:inspect-last, /mason-recap:inspect-session)
...
- **commands/*.md**: 사용자가 실행하는 `/mason-recap:inspect-last`,
  `/mason-recap:inspect-session`, `/mason-recap:status`. `read-events.js`를 호출해 원본
