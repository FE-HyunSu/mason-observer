...
`.mason-observer/events/`에 저장되는 것은 아래뿐이다(전체 필드 목록은
...
전부 `<project-root>/.mason-observer/` 아래에만 저장된다. 플러그인 설치 디렉터리나 플러그인
...
`.mason-observer/`가 심볼릭 링크이고 그 링크가 프로젝트 루트 밖을 가리키는 경우, 수집기는
쓰기를 거부한다(프로젝트 외부로 우회 기록되는 것을 방지).

## 삭제 방법

`.mason-observer/` 디렉터리를 통째로 지우면 모든 로그가 삭제된다. 별도의 삭제 명령이나
...
- 이 플러그인은 로그 파일 자체에 대한 암호화나 OS 수준 접근 제어를 제공하지 않는다.
  `.mason-observer/`는 일반 파일 시스템 권한을 따른다.
